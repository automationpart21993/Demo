...
namespace Machine
{
    partial class DeviceMain
    {
        ...
        public bool Read_Barcode_Retail(string Imei, string Sku, string Upc, string EsimID,bool CCDDetection = true)
        {
            if (CCDDetection)
            {
                if (string.IsNullOrEmpty(Imei) || string.IsNullOrEmpty(Sku) || string.IsNullOrEmpty(Upc))
                    return false;
                ...
                for (int i = 0; i < result.Length; i++)
                {
                    if (!string.IsNullOrEmpty(result[i]))
                    {
                        ...
                        else if (!dataserver.StationLoc[1].EsimIDCheck && result[i].Contains())
                        {   //SkuCheck = true;
                            dataserver.StationLoc[1].UpcCheck = true;
                            Notify(string.Format("{0}:\nSFC-UPC:{1}\nCCD-Barcode:{2}", dataserver.StationLoc[1].DSN, Upc, result[i]));
                            dataserver.StationLoc[1].CCDUpc = result[i];
                ...
                return dataserver.StationLoc[1].IMEICheck && dataserver.StationLoc[1].SkuCheck && dataserver.StationLoc[1].UpcCheck;
            }
            else
            {
                return true;
        ...
        ReCheck_01:
            ...
            count = 0;
        ReCheck_02:
            if (!Read_Barcode_Retail(dataserver.StationLoc[1].IMEI, dataserver.StationLoc[1].Sku, dataserver.StationLoc[1].Upc, ccc.CCDDetection))
            {
            ...
            Save_Window(2, string.Format("{0}Retail_Barcode_{1}", ImageSavePath, DSN));
            dataserver.StationLoc[1].IMEICheck = dataserver.StationLoc[1].SkuCheck = dataserver.StationLoc[1].UpcCheck = true;
            CCD_D02_Running = false;
