...
namespace Machine
{
    ...
    public class HttpHelper
    {
        ...
        private string url_GetDutData = string.Empty;
        ...
        public string Url_GetDutData
        {
            get { return url_GetDutData; }
            set { url_GetDutData = value; }
        }
        ...
        public bool GetDutData(string dutID, out string response)
        {
            try
            {
                string body = "{" + string.Format("\"dut_id\":\"{0}\"", dutID.ToUpper()) + "}";
                response = PostHttp(Url_GetDutData, body);
                if (CheckError(ref response)) return true;
                return false;
            }
            catch (Exception ex)
