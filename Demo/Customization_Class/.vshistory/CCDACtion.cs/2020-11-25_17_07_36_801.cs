...
namespace Machine
{
    partial class DeviceMain
    {
        ...
        public bool Read_Barcode_Retail(string Imei, string Sku, string Upc, bool CCDDetection = true)
        {
            if (CCDDetection)
            {
                ...
                for (int i = 0; i < result.Length; i++)
                {
                    if (!string.IsNullOrEmpty(result[i]))
                    {
                        ...
                        else if (!dataserver.StationLoc[1].UpcCheck && result[i].Equals(Upc))
                        {   //SkuCheck = true;
                            dataserver.StationLoc[1].UpcCheck = true;
                            Notify(string.Format("{0}:\nSFC-UPC:{1}\nCCD-Barcode:{2}", dataserver.StationLoc[1].UPC, Sku, result[i]));
        ...
        ReCheck_01:
            ...
            count = 0;
        ReCheck_02:
            if (!Read_Barcode_Retail(dataserver.StationLoc[1].IMEI, dataserver.StationLoc[1].Sku, ccc.CCDDetection))
            {
            ...
            Save_Window(2, string.Format("{0}Retail_Barcode_{1}", ImageSavePath, DSN));
            dataserver.StationLoc[1].IMEICheck = dataserver.StationLoc[1].SkuCheck = true;
            CCD_D02_Running = false;
