        ...
        ReCheck_02:
            if (!Read_Barcode_Retail(dataserver.StationLoc[1].IMEI, dataserver.StationLoc[1].IMEI2, dataserver.StationLoc[1].Sku, dataserver.StationLoc[1].Upc, dataserver.StationLoc[1].SimID, dataserver.StationLoc[1].EsimID, dataserver.StationLoc[1].SoftwareVersion, ccc.CCDDetection))
            {
                ...
                else
                {
                    ...
                    {
                        notfound[0] = string.Format("[SFCData-IMEI1:{0}]", dataserver.StationLoc[1].IMEI);
                    }
                    if (!dataserver.StationLoc[1].IMEI2Check)
                    {
                        notfound[0] = string.Format("[SFCData-IMEI2:{0}]", dataserver.StationLoc[1].IMEI2);
                    }
                    if (!dataserver.StationLoc[1].SimIDCheck)
                    {
                        notfound[0] = string.Format("[SFCData-SIMID:{0}]", dataserver.StationLoc[1].SimID);
                    }
                    if (!dataserver.StationLoc[1].EsimIDCheck)
                    {
                        notfound[0] = string.Format("[SFCData-ESIMID:{0}]", dataserver.StationLoc[1].EsimID);
                    }
                    if (!dataserver.StationLoc[1].IMEICheck)
                    {
                        notfound[0] = string.Format("[SFCData-IMEI1:{0}]", dataserver.StationLoc[1].IMEI);
